...
class AglareServName3(Converter):
    ...
    def getText(self):
        ...
        if self.type == self.NAME or self.type == self.NAME_ONLY or self.type == self.NAME_EVENT:
            ...
            srpart = "//%s:%s/" % (config.misc.softcam_streamrelay_url.getHTML(),
                                   config.misc.softcam_streamrelay_port.value)
            if not service:
                refstr = info.getInfoString(iServiceInformation.sServiceref)
                path = refstr and eServiceReference(refstr).getPath()
            ...
            return "" if path.startswith(
                "//") and path.find(srpart) == -1 else path
        elif self.type == self.FORMAT_STRING:
            name = self.getName(service, info)
            numservice = hasattr(self.source,
                                 "serviceref") and self.source.serviceref
            ...
            tuner_system = service and info and self.getServiceSystem(
                service, info, tp_data)
            res_str = ""
            for x in self.parts[1:]:
                ...
                if x == "PROVIDER" and provider:
                    res_str = self.appendToStringWithSeparator(
                        res_str, provider)
                if x == "TUNERSYSTEM" and tuner_system:
                    res_str = self.appendToStringWithSeparator(
                        res_str, tuner_system)
            return res_str
    ...
    def changed(self, what):
        if what[0] != self.CHANGED_SPECIFIC or what[1] in (
                iPlayableService.evStart, iPlayableService.evNewProgramInfo):
