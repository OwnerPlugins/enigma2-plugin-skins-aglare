...
class AglareCaidInfo2(Poll, Converter, object):
    ...
    def __init__(self, type):
        ...
        type_tokens = (type or "").split()
        base_type = type_tokens[0] if type_tokens else ""
        if len(type_tokens) > 1:
            self.extra_param = type_tokens[1].upper()
        ...
        if base_type == "CAID":
            ...
            self.type = self.PID
        elif base_type == "ProvID":
            self.type = self.PROV
        elif base_type == "Delay":
        ...
        elif base_type == "CryptInfo":
            self.type = self.CRYPT
        elif base_type == "CryptInfo2":
            self.type = self.CRYPT2
        elif base_type == "BetaCrypt":
            self.type = self.BETA
        elif base_type == "ConaxCrypt":
            self.type = self.CONAX
        elif base_type == "CrwCrypt":
            self.type = self.CRW
        elif base_type == "DreamCrypt":
            self.type = self.DRE
        elif base_type == "ExsCrypt":
            self.type = self.EXS
        elif base_type == "IrdCrypt":
            self.type = self.IRD
        elif base_type == "NagraCrypt":
            self.type = self.NAGRA
        elif base_type == "NdsCrypt":
            self.type = self.NDS
        elif base_type == "SecaCrypt":
            self.type = self.SECA
        elif base_type == "ViaCrypt":
            self.type = self.VIA
        elif base_type == "PwuCrypt":
            self.type = self.PWR
        elif base_type == "VrmCrypt":
            self.type = self.VERI
        elif base_type == "BetaEcm":
            self.type = self.BETA_C
        elif base_type == "ConaxEcm":
            self.type = self.CONAX_C
        elif base_type == "CrwEcm":
            self.type = self.CRW_C
        elif base_type == "DreamEcm":
            self.type = self.DRE_C
        elif base_type == "ExsEcm":
            self.type = self.EXS_C
        elif base_type == "IrdEcm":
            self.type = self.IRD_C
        elif base_type == "NagraEcm":
            self.type = self.NAGRA_C
        elif base_type == "NdsEcm":
            self.type = self.NDS_C
        elif base_type == "SecaEcm":
            self.type = self.SECA_C
        elif base_type == "ViaEcm":
            self.type = self.VIA_C
        elif base_type == "PwuEcm":
            self.type = self.PWR_C
        elif base_type == "VrmEcm":
            self.type = self.VERI_C
        elif base_type == "TanCrypt":
            self.type = self.TAN
        elif base_type == "TanEcm":
            self.type = self.TAN_C
        elif base_type == "BisCrypt":
            self.type = self.BISS
        elif base_type == "BisEcm":
            self.type = self.BISS_C
        elif base_type == "Crd":
            self.type = self.CRD
        elif base_type == "CrdTxt":
            self.type = self.CRDTXT
        elif base_type == "IsFta":
            self.type = self.IS_FTA
        elif base_type == "IsCrypted":
            self.type = self.IS_CRYPTED
        elif base_type == "Short":
            self.type = self.SHORT
        elif base_type == "Default" or base_type == "" or type is None or base_type == "%":
            self.type = self.ALL
        elif base_type == "emuname":
            self.type = self.SOFTCAMNAME
        elif base_type == "emuFullName":
            self.type = self.SOFTCAMFULLNAME
        elif base_type == "caids":
            self.type = self.CAIDS
        elif base_type == "UseCFG":
            self.type = self.USE_CFG
        elif base_type == "ecmfile":
    ...
    def getMyEmuParaSetting(self):
        default_value = "source prov caid ecm_time"
        try:
            return config.plugins.Aglare.myemupara.value or default_value
        except Exception:
            return default_value

    def buildConfiguredShort(self, fields_map):
        combo = self.getMyEmuParaSetting()
        parts = []
        ...
        if not service:
            ...
            if not info:
                ...
                if self.type == self.SHORT:
                    if self.extra_param == "MYEMUPARA":
                        textvalue = self.buildConfiguredShort(formatted_fields)
                    elif source == "emu":
                        textvalue = "%s - %s (Prov: %s, Caid: %s)" % (
                            source, self.systemTxtCaids.get(caid[:2]), prov, caid)
