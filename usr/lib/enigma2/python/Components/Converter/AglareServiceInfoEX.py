...
class AglareServiceInfoEX(Poll, Converter, object):
    ...
    MEDIA_INFO_SHORT = 62  # New merged item for Resolution, FPS
    IS_SOFTCSA = 63  # New item for software CSA (crypt) status
    ...
    def __init__(self, type):
        ...
        elif type == "MediaInfoShort":
            self.type = self.MEDIA_INFO_SHORT
        elif type == "VsizeInfo":
            self.type = self.VSIZE_INFO
        elif type == "IsSoftCSA":
            self.type = self.IS_SOFTCSA
        else:
    ...
    def getText(self):
        ...
        if self.type == self.apid:
        ...
        elif self.type == self.MEDIA_INFO_SHORT:
            width = info.getInfo(iServiceInformation.sVideoWidth)
            height = info.getInfo(iServiceInformation.sVideoHeight)
            progressive = "p" if info.getInfo(
                iServiceInformation.sProgressive) == 1 else "i"
            ...
            return f"{resolution} {fps}fps {video_codec}"
    ...
    def getBoolean(self):
        ...
        if self.type == self.HAS_TELETEXT:
            ...
            # Check if sIsSoftCSA exists (older images may not have it)
            if hasattr(iServiceInformation, 'sIsSoftCSA'):
                return info.getInfo(iServiceInformation.sIsSoftCSA) == 1
            else:
                # Fallback: treat as False or based on some other logic
                return False
