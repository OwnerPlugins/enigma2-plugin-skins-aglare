...
if exists('/usr/lib/bitratecalc.so'):
    if not islink('/usr/lib/enigma2/python/Components/Converter/bitratecalc.so') or not exists(
            '/usr/lib/enigma2/python/Components/Converter/bitratecalc.so'):
        os_system(
            'ln -s /usr/lib/bitratecalc.so /usr/lib/enigma2/python/Components/Converter/bitratecalc.so')
    from Components.Converter.bitratecalc import eBitrateCalculator


if exists('/usr/lib/bitratecalc.so') or exists('/usr/lib/enigma2/python/Components/Converter/bitratecalc.so') or islink(
        '/usr/lib/enigma2/python/Components/Converter/bitratecalc.so'):
    BITRATE = True
...
class AglareEcmInfo(Poll, Converter, object):
            ...
            if BITRATE and vpid > 0 and self.type == self.bitrate:
                try:
                    self.videoBitrate = eBitrateCalculator(
                        vpid, dvbnamespace, tsid, onid, 1000, 1024 * 1024)
            ...
            if BITRATE and apid > 0 and self.type == self.bitrate:
                try:
                    self.audioBitrate = eBitrateCalculator(
                        apid, dvbnamespace, tsid, onid, 1000, 64 * 1024)
    ...
    def getText(self):
        ...
        if self.type == self.vtype:
            try:
                return ("MPEG2", "MPEG4", "MPEG1", "MPEG4-II", "VC1",
                        "VC1-SM", "")[info.getInfo(iServiceInformation.sVideoType)]
            except BaseException:
                return " "
                ...
                if audio:
                    if audio.getCurrentTrack() > -1:
                        if self.audio != 0 or self.video != 0:
                            audioTrackCodec = str(
                                audio.getTrackInfo(
        audio.getCurrentTrack()).getDescription()) or ""
                        else:
                            audioTrackCodec = ""
                else:
                    audioTrackCodec = ""
                yres = info.getInfo(iServiceInformation.sVideoHeight)
                mode = ("i", "p", "")[info.getInfo(
                    iServiceInformation.sProgressive)]
                xres = info.getInfo(iServiceInformation.sVideoWidth)
                return "%sx%s(%s) VIDEO %s: %d kbit/s  AUDIO %s: %d kbit/s" % (str(xres),
                                                                               str(yres) + mode,
                                                                               self.getServiceInfoString(info,
                                                                                                         iServiceInformation.sFrameRate,
                                                                                                         lambda x: "%d" % ((x + 500) / 1000)),
                                                                               ("MPEG2",
                                                                                "MPEG4",
                                                                                "MPEG1",
                                                                                "MPEG4-II",
                                                                               ...
                                                                               audioTrackCodec,
                                                                               self.audio)
                # return "%sx%s(%s) VIDEO %s: %d kbit/s  AUDIO %s: %d kbit/s" %
                # (str(xres), str(yres) + mode, self.getServiceInfoString(info,
                # iServiceInformation.sFrameRate, lambda x: "%d" %
                # ((x+500)/1000)), ("MPEG2", "MPEG4", "MPEG1", "MPEG4-II",
                # "VC1", "VC1-SM",
                # "")[info.getInfo(iServiceInformation.sVideoType)],
                # self.video,str(audio.getTrackInfo(audio.getCurrentTrack()).getDescription()),
                # self.audio)
            except BaseException:
                return " "
        elif self.type == self.txtcaid:
            caidvalue = "%s" % self.systemTxtCaids.get(
                self.caidstr()[:2].upper())
            if caidvalue != "None":
                return caidvalue
            else:
                return " "
            ...
            try:
                return "SID: %0.4X  VPID: %0.4X  APID: %0.4X" % (int(self.getServiceInfoString(info, iServiceInformation.sSID)), int(
                    self.getServiceInfoString(info, iServiceInformation.sVideoPID)), int(self.getServiceInfoString(info, iServiceInformation.sAudioPID)))
            except BaseException:
                try:
                    return "SID: %0.4X  APID: %0.4X" % (int(self.getServiceInfoString(info, iServiceInformation.sSID)), int(
                        self.getServiceInfoString(info, iServiceInformation.sAudioPID)))
                except BaseException:
                    try:
                        return "SID: %0.4X  VPID: %0.4X" % (int(self.getServiceInfoString(info, iServiceInformation.sSID)), int(
                            self.getServiceInfoString(info, iServiceInformation.sVideoPID)))
                    except BaseException:
                        return ""
            ...
            array_caids = []
            try:
                ecminfo = self.getServiceInfoString2(
                    info, iServiceInformation.sCAIDs)
                if ecminfo == "-1":
                    return " "
        ...
        if self.type == self.emuname:
            ...
            if exists(
                resolveFilename(
                    SCOPE_PLUGINS,
                    "SystemPlugins/ViX/SoftcamManager.pyc")):
