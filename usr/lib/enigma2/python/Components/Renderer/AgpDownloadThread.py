...
class AgpDownloadThread(Thread):
    ...
    def _log_provider_title(
            ...
            year=None,
            channel=None):
        """Log provider input vs final cleaned title"""
        try:
            logger.info(
                "[%s] title flow | original='%s' | final='%s' | year='%s' | channel='%s'" %
                (provider_name, str(
                    original_title or ""), str(
                    final_title or ""), str(
                    year or ""), str(
                    channel or "")))
        except Exception:
    ...
    def _select_best_tmdb_result(
        ...
        for each in results:
            ...
            if media_type not in ['movie', 'serie'] or not poster_path:
                continue
    ...
    def search_tmdb(
            ...
            channel=None,
            api_key=None):
        """Download poster from TMDB with full verification pipeline (thread-safe)"""
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    "[tmdb] year flow | step='split_title_and_year_error' | error='{}'".format(
                        str(e)))
                forced_year = ""
                split_title = clean_input_title

            local_title_safe = _strip_year_for_search(split_title)
            logger.info("[tmdb] year flow | step='strip_year_for_search' | title_safe='{}'".format(
            ...
            logger.info("[tmdb] year flow | step='checkType' | srch='{}' | fd='{}'".format(
                str(srch or ""), str(fd or "")))
            ...
            if forced_year and not year:
                year = forced_year
                logger.info("[tmdb] year flow | step='apply_forced_year' | applied_year='{}'".format(
                    str(year or "")))
            else:
            ...
            logger.info("[tmdb] language flow | title='{}' | language='{}'".format(
                str(local_title_safe or ""), str(search_language or "")))
    ...
    def downloadData2(
        ...
        # preserve instance state only inside this call
        old_title_safe = getattr(self, "title_safe", None)
        old_search_year = getattr(self, "search_year", None)
        ...
        try:
            ...
            best, best_score = self._select_best_tmdb_result(
                results, shortdesc, fulldesc, (search_year or self._extract_year(
                    fulldesc or shortdesc or "")))
            ...
            try:
                logger.info(
                    "[tmdb] selected result | title='{}' | original_title='{}' | media_type='{}' | score='{}' | poster_path='{}'".format(
                        str(
                            title or ""), str(
                            best.get(
                                'original_name', best.get(
                                    'original_title', '')) or ""), str(
                            best.get(
                                'media_type', '') or ""), str(best_score), str(
                                    poster_path or "")))
            except Exception:
                pass
    ...
    def search_elcinema(
            ...
            channel=None,
            api_key=None):
        """Download poster from ElCinema using ajax search + work page parsing"""
        self.title_safe = title.replace("+", " ").strip()
        ...
        try:
            ...
            if not year:
                year = self._extract_year(fd)
            self._log_provider_title(
                "elcinema", title, self.title_safe, year, channel)
            ...
            clean_title = _clean_search_title(self.title_safe)
            target_title = _normalize_title(clean_title)
            search_url = "https://elcinema.com/ajaxable/search_simple?q={}".format(
                quoteEventName(clean_title))
            req_headers = {
            ...
            response = http.get(
                search_url, headers=req_headers, timeout=(
                    10, 20), verify=False)
            response.raise_for_status()
            data = response.json()
            if not isinstance(data, list):
                return False, "[SKIP] Invalid ElCinema search response"
            ...
            for html_fragment in data:
                try:
                    ...
                    work_id_match = findall(r'data-id="(\d+)"', html_fragment)
                    if not work_id_match:
                        continue
                    work_id = work_id_match[0]

                    title_match = findall(r'title="([^"]+)"', html_fragment)
                    found_title = title_match[0].strip() if title_match else ""

                    poster_match = findall(
                        r'<img[^>]+src="([^"]+)"', html_fragment)
                    poster_url = poster_match[0] if poster_match else ""

                    media_type_match = findall(
                        r'<li>(فيلم|مسلسل|ﻣﺴﺮﺣﻴﺔ|برنامج|movie|series)</li>', html_fragment)
                    media_type = media_type_match[0] if media_type_match else ""

                    result_year_match = findall(
                        r'<li>(19\d{2}|20\d{2})</li>', html_fragment)
                    result_year = result_year_match[0] if result_year_match else ""

                    found_norm = _normalize_title(found_title)
                    score = 0
                    if found_norm == target_title:
                        score += 500
                    if target_title and target_title in found_norm:
                        score += 120
                    if found_norm and found_norm in target_title:
                        score += 80
                    if poster_url and "blank_photos" not in poster_url:
                        score += 100
                    if chkType == "movie" and media_type in ("فيلم", "movie"):
                        score += 80
                    elif chkType == "tv" and media_type in ("مسلسل", "series"):
                        score += 80
                    elif chkType == "multi" and media_type in ("فيلم", "مسلسل", "movie", "series"):
                        score += 20
                    if year:
            ...
            if not candidates:
                return False, "[SKIP] No ElCinema candidates found"

            candidates.sort(key=lambda x: x["score"], reverse=True)
            best = candidates[0]
            if best["score"] < 50:
                return False, "[SKIP] No valid ElCinema match"

            work_url = "https://elcinema.com/work/{}".format(best["id"])
            work_response = http.get(
                work_url, headers=req_headers, timeout=(
                    10, 20), verify=False)
            ...
            if not img_match:
                img_match = findall(
                    r'<img[^>]+src="(https?://[^"]+_315x420_[^"]+)"', work_html)
            if not img_match:
                img_match = findall(
                    r'<img[^>]+src="(https?://[^"]+\.(?:jpg|jpeg|png))"', work_html)
            ...
            return False, "[SKIP] No poster found on ElCinema work page"

        except HTTPError as e:
            logger.error("[ElCinema] HTTP Error: {}".format(str(e)))
            return False, "[ERROR] ElCinema HTTP error"

        except ValueError as e:
            logger.error("[ElCinema] JSON/Error: {}".format(str(e)))
            return False, "[ERROR] ElCinema response parse failed"
