...
class AgpDownloadThread(Thread):
	...
	def _log_provider_title(self, provider_name, original_title, final_title, year=None, channel=None):
	...
	def downloadData2(self, data, dwn_poster, shortdesc="", fulldesc="", title_safe="", search_year=""):
		...
		try:
			...
			try:
				logger.info("[tmdb] selected result | title='{}' | original_title='{}' | media_type='{}' | score='{}' | poster_path='{}'".format(
					str(title or ""),
	...
	def search_elcinema(self, dwn_poster, title, shortdesc, fulldesc, year=None, channel=None, api_key=None):
		...
		try:
			...
			if not year:
				year = self._extract_year(fd)
			self._log_provider_title("elcinema", title, self.title_safe, year, channel)
			...
			response = http.get(search_url, headers=req_headers, timeout=(10, 20), verify=False)
			response.raise_for_status()
			data = response.json()
			...
			for html_fragment in data:
				try:
					...
					poster_match = findall(r'<img[^>]+src="([^"]+)"', html_fragment)
					poster_url = poster_match[0] if poster_match else ""

					media_type_match = findall(r'<li>(فيلم|مسلسل|ﻣﺴﺮﺣﻴﺔ|برنامج|movie|series)</li>', html_fragment)
					media_type = media_type_match[0] if media_type_match else ""

					result_year_match = findall(r'<li>(19\d{2}|20\d{2})</li>', html_fragment)
			...
			work_response = http.get(work_url, headers=req_headers, timeout=(10, 20), verify=False)
			work_response.raise_for_status()
			work_html = work_response.text

			img_match = findall(r'<meta\s+property="og:image"\s+content="([^"]+)"', work_html)
			if not img_match:
				img_match = findall(r'<img[^>]+src="(https?://[^"]+_315x420_[^"]+)"', work_html)
			if not img_match:
				img_match = findall(r'<img[^>]+src="(https?://[^"]+\.(?:jpg|jpeg|png))"', work_html)

			if img_match:
				poster = img_match[0]
				callInThread(self.savePoster, poster, dwn_poster)
				if exists(dwn_poster):
					return True, "[SUCCESS] Poster match: {} ({})".format(best["title"], best["id"])
