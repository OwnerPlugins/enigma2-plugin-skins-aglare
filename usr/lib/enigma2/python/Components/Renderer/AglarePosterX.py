...
class AglarePosterX(Renderer):
	...
	def __init__(self):
		...
		self.pstrNm = None
		self.backrNm = None
		self.poster_refresh_count = 0
		self.poster_refresh_max = 20
		self.poster_refresh_delay = 300
		...
		self.show_timer.callback.append(self.refreshPoster)
		...
		for attrib, value in self.skinAttributes:
			if attrib == "nexts":
				...
				try:
					...
					logger.error("Failed to create poster path {}: {}".format(self.storage_path, str(e)))
			else:
	...
	def changed(self, what):
		...
		try:
			...
			if source_type is ServiceEvent:
				...
				if self.nxts:
					...
					if getattr(source, "event", None) is None:
						logger.info("AglarePosterX: Event source has no event yet")
						if self.instance:
							self.instance.hide()
						return
					...
					if not event_name:
						logger.info("AglarePosterX: Event source has empty event name")
						if self.instance:
							self.instance.hide()
						return
			...
			if checkPosterExistence(poster_path):
				if not self.showPoster(poster_path):
					try:
						self.show_timer.start(self.poster_refresh_delay, True)
					except Exception:
						pass
			else:
				pdb.put(self.canal[:])
	...
	def showPoster(self, poster_path=None):
		...
		if not poster_path or not checkPosterExistence(poster_path):
			self.instance.hide()
			return False
		...
		try:
			...
			logger.warning("Poster load failed | file='{}' | error='{}'".format(poster_path, str(e)))
	...
	def refreshPoster(self):
		...
		if self.showPoster(self.backrNm):
			self.poster_refresh_count = 0
			return

		self.poster_refresh_count += 1

		if self.poster_refresh_count < self.poster_refresh_max:
			try:
				self.show_timer.start(self.poster_refresh_delay, True)
			except Exception:
				pass
		else:
			logger.info("Poster refresh stopped | file='{}'".format(self.backrNm))
			self.poster_refresh_count = 0
	...
	def waitPoster(self):
		...
		poster_path = join(self.storage_path, "%s.jpg" % pstcanal)
...
class PosterDB(AgpDownloadThread):
	...
	def __init__(self, providers=None):
		...
		logger.info("PosterDB executor configured | max_workers='1'")
		...
		try:
			...
			try:
				...
				for provider_name, provider_func in sorted_providers:
					try:
						# Only providers in API_CONFIG require keys; keyless providers must run normally
						api_key = api_key_manager.get_api_key(provider_name)
						if provider_name in api_key_manager.API_CONFIG and not api_key:
