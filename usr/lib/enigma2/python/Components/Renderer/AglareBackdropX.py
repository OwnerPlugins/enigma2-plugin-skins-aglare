...
class AglareBackdropX(Renderer):
	...
	def __init__(self):
		...
		self.pstrNm = None
		self.backrNm = None
		self.backdrop_refresh_count = 0
		self.backdrop_refresh_max = 20
		self.backdrop_refresh_delay = 300
		...
		self.show_timer.callback.append(self.refreshBackdrop)
		...
		for attrib, value in self.skinAttributes:
			if attrib == "nexts":
				...
				try:
					...
					logger.error("Failed to create backdrop path {}: {}".format(self.storage_path, str(e)))
			else:
	...
	def changed(self, what):
		...
		try:
			...
			if source_type is ServiceEvent:
				...
				if self.nxts:
					...
					if getattr(source, "event", None) is None:
						logger.info("AglareBackdropX: Event source has no event yet")
						if self.instance:
							self.instance.hide()
						return
					...
					if not event_name:
						logger.info("AglareBackdropX: Event source has empty event name")
						if self.instance:
							self.instance.hide()
						return
			...
			self.pstcanal = get_search_title(self.canal[5], self.canal[4], self.canal[3])
			if not self.pstcanal:
				return

			skip_title, skip_word = should_skip_title(self.pstcanal)
			if skip_title:
				logger.info(f"Skipping before queue: original='{self.canal[2]}' | final_search_title='{self.pstcanal}' | matched_exclusion='{skip_word}' | channel='{self.canal[0]}'")
				return
			...
			if self.pstcanal in self.backdrop_cache:
				...
				if checkBackdropExistence(cached_path):
					self.backrNm = cached_path
					self.backdrop_refresh_count = 0
					if not self.showBackdrop(cached_path):
						try:
							self.show_timer.start(self.backdrop_refresh_delay, True)
						except Exception:
							pass
					return

			# Try to display existing backdrop, or keep refreshing until download finishes
			...
			if checkBackdropExistence(backdrop_path):
				if not self.showBackdrop(backdrop_path):
					try:
						self.show_timer.start(self.backdrop_refresh_delay, True)
					except Exception:
						pass
	...
	def showBackdrop(self, backdrop_path=None):
		...
		if not backdrop_path or not checkBackdropExistence(backdrop_path):
			self.instance.hide()
			return False
		...
		try:
			...
			logger.warning("Backdrop load failed | file='{}' | error='{}'".format(
				backdrop_path, str(e)
			))
	...
	def refreshBackdrop(self):
		...
		if self.showBackdrop(self.backrNm):
			self.backdrop_refresh_count = 0
			return

		self.backdrop_refresh_count += 1

		if self.backdrop_refresh_count < self.backdrop_refresh_max:
			try:
				self.show_timer.start(self.backdrop_refresh_delay, True)
			except Exception:
				pass
		else:
			logger.info("Backdrop refresh stopped | file='{}'".format(self.backrNm))
			self.backdrop_refresh_count = 0


	def waitBackdrop(self):
		"""Start/continue refresh loop for expected backdrop path"""
		if not self.instance or not self.canal[5]:
			return

		pstcanal = get_search_title(self.canal[5], self.canal[4], self.canal[3])
		backdrop_path = join(self.storage_path, "%s.jpg" % pstcanal)
...
class BackdropDB(AgbDownloadThread):
	...
	def __init__(self, providers=None):
		...
		logger.info("BackdropDB executor configured | max_workers='1'")
	...
	def _process_canal_task(self, canal):
		...
		try:
			self.pstcanal = get_search_title(canal[5], canal[4], canal[3])
			if not self.pstcanal:
			...
			skip_title, skip_word = should_skip_title(self.pstcanal)
			if skip_title:
				logger.info(f"Skipping title: original='{canal[2]}' | final_search_title='{self.pstcanal}' | matched_exclusion='{skip_word}' | channel='{canal[0]}'")
				return
			...
			try:
				...
				logger.info(
					f"Starting download: original='{canal[2]}' | clean_for_tvdb='{clean_for_tvdb(canal[5])}' | final_search_title='{self.pstcanal}' | channel='{canal[0]}'"
				)
...
class BackdropAutoDB(AgbDownloadThread):
	...
	def _pre_download_checks(self, canal):
		...
		self.pstcanal = get_search_title(canal[5] or "", canal[4], canal[3])
		if not self.pstcanal:
			return False

		skip_title, skip_word = should_skip_title(self.pstcanal)
		if skip_title:
			logger.info(f"Skipping auto-download: original='{canal[2]}' | final_search_title='{self.pstcanal}' | matched_exclusion='{skip_word}' | channel='{canal[0]}'")
			return False
