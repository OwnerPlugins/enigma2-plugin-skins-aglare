...
class AglareBackdropX(Renderer):
        ...
        for attrib, value in self.skinAttributes:
            if attrib == "nexts":
                ...
                try:
                    ...
                    logger.error(
                        "Failed to create backdrop path {}: {}".format(
    ...
    def changed(self, what):
        ...
        try:
            ...
            self.pstcanal = get_search_title(
                    ...
                    if not self.showBackdrop(cached_path):
                        try:
                            self.show_timer.start(
                                self.backdrop_refresh_delay, True)
                        except Exception:
                            pass
                    return

            # Try to display existing backdrop, or keep refreshing until
            # download finishes
    ...
    def showBackdrop(self, backdrop_path=None):
        ...
        try:
            ...
            logger.warning(
                "Backdrop load failed | file='{}' | error='{}'".format(
                    backdrop_path, str(e)))
    ...
    def refreshBackdrop(self):
        ...
        if self.backdrop_refresh_count < self.backdrop_refresh_max:
            ...
            logger.info(
                "Backdrop refresh stopped | file='{}'".format(
                    self.backrNm))
            self.backdrop_refresh_count = 0
